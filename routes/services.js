...
// ========================================
// PUBLIC / MAIN ADMIN ROUTES (No auth)
// ========================================
...
// UPDATE service - for main admin (MUST BE BEFORE /:id with auth middleware)
...
router.get('/branch', authenticateBranchAdmin, checkPermission('manage_services'), async (req, res) => {
  try {
    ...
    console.log(`✅ GET /api/services/branch → ${services.length} services for branch ${req.branchId}`);
    res.json(services);
  } catch (error) {
...
router.post('/branch', authenticateBranchAdmin, checkPermission('manage_services'), async (req, res) => {
  try {
    ...
    // Check duplicate in this branch
    const existingService = await Service.findOne({ 
      name: name.trim(), 
      gender: gender.toLowerCase(),
    ...
    if (existingService) {
      return res.status(400).json({ 
        message: `Service "${name}" already exists in your branch` 
      });
    }
    ...
    await service.save();
    const populated = await Service.findById(service._id).populate('branches', 'name');
    console.log('✅ Branch service created:', service._id, '-', service.name);
    res.status(201).json(populated);
  } catch (error) {
    console.error('❌ CREATE branch service error:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
router.put('/branch/:id', authenticateBranchAdmin, checkPermission('manage_services'), async (req, res) => {
  try {
    ...
    // Check if branch has permission
    const branchIds = service.branches.map(b => b.toString());
    if (!branchIds.includes(req.branchId.toString())) {
    ...
    const updateData = {
      ...
      duration: duration?.trim() || service.duration,
      price: price?.trim() || service.price,
      gender: gender?.toLowerCase() || service.gender,
      branches: branches || service.branches
    };
    ...
    console.log('✅ Branch service updated:', updated._id);
    res.json(updated);
  } catch (error) {
    console.error('❌ UPDATE branch service error:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
router.delete('/branch/:id', authenticateBranchAdmin, checkPermission('manage_services'), async (req, res) => {
  try {
    ...
    console.log('✅ Branch service deleted:', service._id);
    res.json({ message: 'Service deleted successfully' });
  } catch (error) {
    console.error('❌ DELETE branch service error:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
